...
lgb_best_ens <- my_lgb(lgb_dtrain_ens)
...
test_ens <- test_ind_res %>%
  mutate(xgb_ens = predict(xgb_best_ens, dtest_ens),
         lgb_ens = predict(lgb_best, lgb_dtest_ens),
...
## write out
```{r}
write.csv(final_out, "../../../data/predictions/lk_predictions/train_with_leak/lk_top50rfxgb_6ens_lv1.csv",
          row.names = FALSE)
```
