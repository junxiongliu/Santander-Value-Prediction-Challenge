...
# New way of stacking, training (at level 2) includes the log-leaky column
...
# combine train_leak_whole with train_for_ens
```{r}
...
all_ind_last_round <- c()
for (i in 1:length(ind)){
  all_ind_last_round <- c(all_ind_last_round, ind[[i]])
}

train_for_ens <- train_for_ens %>%
  mutate(row_num = all_ind_last_round)

# join
train_for_ens <- left_join(train_for_ens, train_leak_cleaned, by = "row_num")
```
...
ensemble_features_1 <- c(top_feas, c("xgb", "rf", "knn", "lm", "ridge", "lgb", "compiled_leak")) #,...), "SVMLinear"
...
## manipulate and combine the test set
```{r}
...
lgb_dtest_r1 <- as.matrix(test_r1[,intersect_features_1])
...
## write out
```{r}
write.csv(final_out, "../../../data/predictions/lk_predictions/train_with_leak/lk_top100rfxgb_6ens_lv1.csv",
          row.names = FALSE)
```
